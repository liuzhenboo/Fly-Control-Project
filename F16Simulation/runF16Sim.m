...
disp(newline);
 %deg角度；AOA攻角；Altitude海拔；Thrust推力；lbs磅；Aileron副翼；Rudder方向舵；ft英尺
disp('                                  Model');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Initialize some varibles used to create disturbances（扰动）. %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if dis_flag == 'y' 
  ElevatorDis = input('Enter the elevator（升降舵） distrubance deflection（偏转）           (deg) :  ');
  DisEle_1 = ElevatorDis;    DisEle_2 = -2*ElevatorDis;    DisEle_3 = ElevatorDis; surfacedef = 'elevator';
  
  AileronDis = input('Enter the aileron（副翼） distrubance deflection            (deg) :  ');
  DisAil_1 = AileronDis;    DisAil_2 = -2*AileronDis;    DisAil_3 = AileronDis; surfacedef = 'aileron';
  
  RudderDis = input('Enter the rudder（方向舵） distrubance deflection             (deg) :  ');
...
%trim_file是一个文件名字：ele_'ElevatorDis'ail_'AileronDis'rud_'RudderDis'_'fi_type'_alt'altitude'_vel'velocity'，它用来存放仿真结果（画图会用到这个结果）
